...
WString::WString(const wchar_t *str, size_t len) : length(len) {
...
WString::WString(wchar_t ch, size_t count) : length(count) {
  first = alloc(length + 1);
  last = first + length - 1;
...
WString::WString(wchar_t ch) : length(1) {
  first = alloc(2);
  last = first;
  *first = ch;
  *(last + 1) = 0;
...
WString WString::ToLower() {
  ...
  for (size_t i = 0; i < length; i++) ret[i] = towlower(first[i]);
  ret[length] = 0;
...
WString WString::ToUpper() {
  wchar_t *ret = new wchar_t[length + 1];
  for (size_t i = 0; i < length; i++) ret[i] = towupper(first[i]);
  ret[length] = 0;
  return WString(ret, length, false);
}
...
WString WString::Title() {
  ...
  *ret = towupper(*ret);
  for (size_t i = 0; i < length - 1; i++)
    if (first[i] == L' ') ret[i + 1] = towupper(first[i + 1]);
  return WString(ret, length, false);
}
...
WString WString::Remove(size_t starts, size_t len) {
  ...
  memcpy(newString + starts, first + starts + len,
         (retlen - starts) * sizeof(wchar_t));
...
WString WString::Slicing(size_t jmp, size_t starts, size_t len, bool remain) {
...
char *WString::ToAnsi() { return UnicodeToAnsi(); }
...
WString WString::AppendHelper(const wchar_t *str, size_t len) {
  ...
  if (length > 0) memcpy(appendString, first, length * sizeof(wchar_t));
...
size_t WString::FindLastHelper(const wchar_t *str, size_t ends,
                               size_t len) const {
...
bool WString::ContainsHelper(const wchar_t *str, size_t len,
                             bool ignore) const {
...
#define _CONTAINS_HELPER_SKIP(t, ch)                                     \
  {                                                                      \
    if (to_wlower(t[len_1]) == strlast) {                                \
      s1 = t + 1;                                                        \
      s2 = str + 1;                                                      \
      while (*s1 && *s2 && !(towlower(*s1) - towlower(*s2))) s1++, s2++; \
      if (*s2 == 0) return true;                                         \
    }                                                                    \
    t = wcschr(t + 1, ch);                                               \
  }
...
WString::ArrayType WString::SplitHelper(const wchar_t *src, size_t srclen,
                                        size_t max) {
...
WString::ArrayType WString::SplitSlowHelper(const wchar_t *src, size_t srclen,
                                            size_t max) {
  size_t max_dec = max;
  size_t count = 0;
  size_t i = 0;
...
WString WString::SplitPositionHelper(const wchar_t *src, size_t srclen,
                                     size_t pos) {
  wchar_t *ptr = first, *tptr;
...
WString::ArrayType WString::SplitReverseHelper(const wchar_t *src,
  ...
  for (; (tptr = StringTools::wcsrnstrn(first, nowlen, src, srclen)) && max;
       max--, count++) {
...
WString WString::BetweenHelper(const wchar_t *left, size_t llen,
                               const wchar_t *right, size_t rlen,
                               size_t starts) {
  size_t lefts = FindFirst(left, starts);
  size_t rights = FindFirst(right, lefts);

  if ((lefts == error) || (rights == error)) return WString();

  lefts += llen;

  if (lefts > rights) std::swap(lefts, rights);

  return Slice(lefts, rights - 1);
}

WString::ArrayType WString::BetweensHelper(const wchar_t *left, size_t llen,
                                           const wchar_t *right, size_t rlen,
                                           size_t starts) {
...
bool WString::StartsWithHelper(const wchar_t *str, size_t starts,
  ...
  if (length < len + starts) return false;

  return !memcmp(first + starts, str, len * sizeof(wchar_t));
}

bool WString::EndsWithHelper(const wchar_t *str, size_t ends,
                             size_t len) const {
  if (ends >= length) return false;

  if (length < len - ends) return false;

  return !memcmp(last - ends - len + 1, str, len * sizeof(wchar_t));
}

WString WString::InsertLeftHelper(size_t separation, const wchar_t *str,
                                  size_t strlen) {
...
WString WString::InsertRightHelper(size_t separation, const wchar_t *str,
                                   size_t strlen) {
  size_t sizeof_diff = (length - 1) / separation;
  size_t len = sizeof_diff * strlen + length;
  size_t dstrlen = strlen * sizeof(wchar_t);
...
WString WString::ReplaceSlowHelper(const wchar_t *src, const wchar_t *dest,
  ...
  for (ptr = first; (tptr = wcsstr(ptr, src)) && max; max--,
...
WString WString::InsertHelper(size_t starts, const wchar_t *str, size_t len) {
  ...
  if (newLen == 0) return *new WString();

  wchar_t *newString = new wchar_t[newLen + 1];

  memcpy(newString, first, starts * sizeof(wchar_t));
  memcpy(newString + starts, str, len * sizeof(wchar_t));
  memcpy(newString + starts + len, first + starts,
         (length - starts) * sizeof(wchar_t));
...
WString::ArrayType WString::LineSplitHelper(size_t len, const wchar_t *front,
  ...
  for (size_t i = 0; i < countLineLen; i++) {
    ...
    if (front_len) memcpy(partialLine, front, dfront_len);
    memcpy(partialLine + front_len, first + i * len, dlen);
    if (end_len) memcpy(partialLine + front_len + len, end, dend_len);
  ...
  if (remainLen != 0) {
    if (end_len) {
      StringTools::wcsnset(n[countLineLen - 1]->first + front_len + remainLen,
                           L' ', len - remainLen);
    } else {
