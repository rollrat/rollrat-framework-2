...
WString ofw::Base64Encoding::Base64Encode(unsigned char *bytes, size_t size) {
  ...
  if (size > 1) {
    for (; i < size - 2; i += 3) {
      ...
      wb.Append(
          table[((bytes[i] & 0x03) << 4) | ((*(bytes + i + 1) & 0xf0) >> 4)]);
...
Base64Encoding::Base64Array ofw::Base64Encoding::Base64Decode(
  ...
  if (size % 4 != 0) return Base64Encoding::Base64Array(nullptr, 0);
  ...
  unsigned char *ret =
      new unsigned char[totalsz = (size * 3) / 4 - ((wchs[size - 1] == L'=') +
                                                    (wchs[size - 2] == L'='))];
...
Base64Encoding::Base64Array ofw::Base64Encoding::Base64Decode(const char *chs,
                                                              size_t size) {
  if (size % 4 != 0) return Base64Encoding::Base64Array(nullptr, 0);
  ...
  unsigned char *ret =
      new unsigned char[totalsz = (size * 3) / 4 - ((chs[size - 1] == L'=') +
                                                    (chs[size - 2] == L'='))];
  unsigned char *ptr = ret;
