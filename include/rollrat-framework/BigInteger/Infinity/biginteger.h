...
class basic_integer {
 ...
 public:
  basic_integer(const std::string str) { string_to_bigint(str); }

  basic_integer(const char* str) : basic_integer(std::string(str)) {}
  ...
  std::string operator*() const { return bigint_to_string(engine.load(0)); }

  bit& operator[](size_t off) { return engine.load(0)[off]; }

  bit operator[](size_t off) const { return engine.load(0)[off]; }
  ...
  this_type& operator++() { engine.add(0, integer_bits({1})); }
  ...
  bool operator>(const this_type& integer) const { return integer < *this; }

  bool operator<=(const this_type& integer) const { return !(integer < *this); }

  bool operator>=(const this_type& integer) const { return !(*this < integer); }
  ...
  this_type gcd(const this_type& integer) const {
    ...
    while (true) {
      ...
      if (tmp.engine._register[2][0] == off) break;
      tmp.engine.eql(0, 1, 2);
      if (tmp.engine._register[2][0] == on) break;
  ...
  this_type power(const this_type& integer) const {
    ...
    if (tmp.engine.load(1).zero() == off) {
      while (true) {
        ...
        if (tmp.engine.load(1).zero() == on) break;
 ...
 private:
  ...
  std::string bits_to_string(integer_bits bits) const {
    ...
    for (; i >= 0; i -= 4) {
      if (bits[i - 1] == on || bits[i - 2] == on || bits[i - 3] == on ||
          bits[i - 4] == on) {
        break;
      }
    }
    ...
    if (i <= 0)
      ...
      for (; i > 0; i -= 4)
        ak += '0' + ((bits[i - 1].var() << 3) | (bits[i - 2].var() << 2) |
                     (bits[i - 3].var() << 1) | (bits[i - 4].var() << 0));
