...
void WStringBuilder::Append(wchar_t ch) {
  ...
  if (m_last->m_length == capacity - 1) Expand();
...
WString WStringBuilder::ToString() {
  ...
  WStringBuilderNode *iter = m_head;
  for (; iter != nullptr; iter = iter->m_next) {
    memcpy(newString + iter->m_offset, iter->m_ptr,
           iter->m_length * sizeof(wchar_t));
  }
  newString[len] = 0;
...
bool WStringBuilder::MatchContinue(const WString &str, size_t start_offset,
                                   WStringBuilderNode *start_node,
                                   size_t node_offset) {
  if (start_node == nullptr) {
    if (str.Length() == start_offset) return true;
    return false;
  }

  if (str.Length() - start_offset < start_node->m_length - node_offset) {
    if (WString(start_node->m_ptr, start_node->m_length)
...
WStringBuilder::WStringBuilderNode *WStringBuilder::ReplaceInternal(
    const WString &src, const WString &tar, WStringBuilderNode *start_node,
    size_t node_offset) {
  ...
  if (node_offset + src.Length() <= start_node->m_length) {
    ...
    memcpy(
        node1->m_ptr, start_node->m_ptr + node_offset + src.Length(),
