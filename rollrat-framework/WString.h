...
class WString final {
 ...
 public:
  ...
  WString(float);
  WString(double);
  WString(WString &&ws) : first(ws.first), last(ws.last), length(ws.length) {
    ws.tm = true;
  }
  WString(const WString &cnt)
      : WString((const wchar_t *)cnt.first, cnt.length) {}
  ...
  inline bool Equal(const WString &refer) const {
    if (refer.length != this->length) return false;
 ...
 private:
  ...
  bool ContainsHelper(const wchar_t *str, size_t len,
  ...
  ArrayType BetweensHelper(const wchar_t *left, size_t llen,
  ...
  WString ReplaceHelper(const wchar_t *src, const wchar_t *dest, size_t srclen,
  ...
  ArrayType LineSplitHelper(size_t len, const wchar_t *front, size_t front_len,
                            const wchar_t *end, size_t end_len);
 ...
 public:
  ...
  WString Between(const wchar_t *left, const wchar_t *right,
                  size_t starts = 0) {
    return BetweenHelper(left, StringTools::wcslen(left), right,
                         StringTools::wcslen(right), starts);
  }
  WString Between(const WString &left, const WString &right,
                  size_t starts = 0) {
  ...
  inline bool StartsWith(const wchar_t ch) const { return *first == ch; }
  ...
  inline bool EndsWith(const wchar_t ch) const { return *last == ch; }
  ...
  WString Replace(const wchar_t *src, const wchar_t *dest,
                  size_t max = SIZE_MAX) {
    return ReplaceHelper(src, dest, StringTools::wcslen(src),
                         StringTools::wcslen(dest), max);
  }
  ...
  WString ReplaceSlow(const wchar_t *src, const wchar_t *dest,
                      size_t max = SIZE_MAX) {
    return ReplaceSlowHelper(src, dest, StringTools::wcslen(src),
                             StringTools::wcslen(dest), max);
  }
  ...
  ArrayType LineSplit(size_t len, const wchar_t *front) {
    return LineSplitHelper(len, front, StringTools::wcslen(front), nullptr, 0);
  }
  ...
  ArrayType LineSplit(size_t len, const wchar_t *front, const wchar_t *end) {
    return LineSplitHelper(len, front, StringTools::wcslen(front), end,
                           StringTools::wcslen(end));
  }
...
inline WString operator"" _ws(unsigned long long i) { return WString{i}; }
